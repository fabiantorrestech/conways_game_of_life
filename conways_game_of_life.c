...
// ********* struct type needed for linked list **********
...
int neighbors(struct node *begin , int row, int col){
	...
	if( ( row == 0 && col == 0) || ( row == 0 && col == (begin->columns - 1) ) || (row == (begin->rows -1) && col== 0) || (row == (begin->rows - 1) && col == (begin -> columns - 1)) ){
		livingNeighbors = NeighborCorners(begin, row, col); 
	}
	else if( ( row == 0 && col <= (begin -> columns - 2) ) || ( row == (begin-> rows - 1) && col <= (begin-> columns - 2) ) || (row <=(begin-> rows-2) && col== 0) || (row <= (begin-> rows - 2) && col == (begin->columns - 1) )){//check if the cell is by the border
		livingNeighbors = NeighborEdges(begin, row, col); 
	}
	else{
		livingNeighbors = *(begin->grid + (begin->columns)* (row - 1) + (col - 1)) + *(begin->grid + (begin->columns)* (row - 1) + (col))+ *(begin->grid + (begin->columns)* (row - 1) + (col + 1))+ *(begin->grid + (begin->columns)* (row) + (col - 1));
		livingNeighbors = livingNeighbors + *(begin->grid + (begin->columns)* (row) + (col + 1)) + *(begin->grid + (begin->columns)* (row + 1) + (col - 1)) + *(begin->grid + (begin->columns)* (row + 1) + (col)) + *(begin->grid + (begin->columns)* (row + 1) + (col + 1));
	}
	return livingNeighbors;
}

// for the borders of the grid. it should return the number of alive neighbors in the corners of the grid.
int NeighborCorners(struct node *begin, int row, int col){

	int livingNeighbors = 0; 

	if( row == 0 && col == 0){ 
		livingNeighbors = *(begin->grid + (begin->columns)* (row + 1) + (col)) + *(begin->grid + (begin->columns)* (row) + (col + 1)) + *(begin->grid + (begin->columns)* (row + 1) + (col + 1));
	}
	else if( row == 0 && col == (begin->columns - 1) ){
		livingNeighbors = *(begin->grid + (begin->columns)* (row) + (col - 1)) + *(begin->grid + (begin->columns)* (row + 1) + (col - 1)) + *(begin->grid + (begin->columns)* (row + 1) + (col));
	}
	else if (row == (begin->rows - 1) && col == 0){
		livingNeighbors = *(begin->grid + (begin->columns)* (row - 1) + (col)) + *(begin->grid + (begin->columns)* (row) + (col + 1)) + *(begin->grid + (begin->columns)* (row - 1) + (col + 1));
	}
	else if (row == (begin->rows - 1) && col == (begin->columns - 1)){
		livingNeighbors = *(begin->grid + (begin->columns)* (row - 1) + (col - 1)) + *(begin->grid + (begin->columns)* (row) + (col - 1)) + *(begin->grid + (begin->columns)* (row - 1) + (col));	
	}
...
int NeighborEdges(struct node *begin, int row, int col){
	...
	int livingNeighbors = 0 ; //initialize the livingNeighbors

	// top edges
	if (row == 0){
		livingNeighbors = *(begin->grid + (begin->columns)* (row) + (col - 1)) + *(begin->grid + (begin->columns)* (row + 1) + (col - 1)) + *(begin->grid + (begin->columns)* (row + 1) + (col)) + + *(begin->grid + (begin->columns)* (row + 1) + (col + 1)) + *(begin->grid + (begin->columns)* (row) + (col + 1));
	}
	// bottom edges
	else if (row == (begin->rows - 1)){
		livingNeighbors = *(begin->grid + (begin->columns)* (row - 1) + (col - 1)) + *(begin->grid + (begin->columns)* (row - 1) + (col)) + *(begin->grid + (begin->columns)* (row - 1) + (col + 1)) + + *(begin->grid + (begin->columns)* (row) + (col - 1)) + *(begin->grid + (begin->columns)* (row) + (col + 1));
	}
	// left edge
	else if (col == 0){
		livingNeighbors = *(begin->grid + (begin->columns)* (row - 1) + (col)) + *(begin->grid + (begin->columns)* (row + 1) + (col)) + *(begin->grid + (begin->columns)* (row - 1) + (col + 1)) + + *(begin->grid + (begin->columns)* (row) + (col + 1)) + *(begin->grid + (begin->columns)* (row+1) + (col + 1));
	}
	// right edge
...
struct node *createInitialNode(FILE *input, int *numsteps){
	...
	while(fscanf(input, "%d %d", &x, &y) != EOF){
		...
		// checking if x and y go over the user's input for rows and columns because it can't populate where it doensn't exist.
		if(x > begin->rows){
...
void nextStep(struct node *begin){
	...
	// add a struct node to the end of the list pointed to by begin for the next time step, so use data in the last node in the list to create the new timestep. 
...
void printList(struct node *begin){
	...
	int position; //  'coordinate' from createinitalnode function = position here //  equivalent to checking [i+1][j], [i][j+1], etc...; we will use temp's columns * i (always 1 less than number of rows) + j ( always one less than the number of columns)

	// while temp's node's contents are still full, which will happen for as many nodes 
	while(begin != NULL){
		for(i=0; i < begin->rows; i++){
			for(j=0; j < begin->columns; j++){
...
int main (){
	...
	if (inFile==NULL){
        printf("ERROR opening filename %sant.txt", &given_file_name); // Checks if the file is empty
        return 0;
    }


	//reading in the numsteps given by user; remember that numsteps is a POINTER INT, not a regular int.
	int numsteps;
